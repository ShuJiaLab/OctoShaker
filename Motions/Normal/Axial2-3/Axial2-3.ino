...
int revolutions = 5; // total number of revolutions !!![obsolete]  // replaced by iterations
...
int inter_axis_delay = 300; //Organoids originnal: 200 um --> delayMicroseconds()
int inter_motor_delay = 1300; // Organoids originnal: 900 um  --> delayMicroseconds()
int iter_time = (8 * inter_axis_delay + 2 * inter_motor_delay * amp * 8)/ 1000; //ms per iteration
int run_time = 15 * 60 * 1000; // 15 min * 60 seconds * 1000 s/ms
int iterations = floor(run_time/iter_time);
unsigned long startTime;
unsigned long time_iteration; //73.6ms per revolution
// Run 57 iterations for 15 min
...
void axial23() { // each motor is given 30 pulses per revolution // double check
